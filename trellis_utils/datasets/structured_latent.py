...
from .components import StandardDatasetBase, TextConditionedMixin, ImageConditionedMixin, CustomDatasetBase, SlatConditionedMixin, SlatTextConditionedMixin
...
class CustomSLat(SLatVisMixin, CustomDatasetBase):
    def __init__(self,
        roots: str,
        *,
        latent_model: str,
        ...
        self.loads = [1 for _ in range(len(self.instances))]
    ...
    def get_instance(self, root, scene_id, frame_id, frame_cond_id):
        data = np.load(os.path.join(root, scene_id, frame_id, 'latents.npz'))
        coords = torch.tensor(data['coords']).int()
        feats = torch.tensor(data['feats']).float()
        if self.normalization is not None:
            feats = (feats - self.mean) / self.std
    ...
    def collate_fn(batch, split_size=None):
        ...
        for group in group_idx:
            ...
            # collate cond
            pack['cond'] = [b['cond'] for b in sub_batch]

            keys = [k for k in sub_batch[0].keys() if k not in ['coords', 'feats', 'cond']]
            for k in keys:
                if isinstance(sub_batch[0][k], torch.Tensor):
                    pack[k] = torch.stack([b[k] for b in sub_batch])
                elif isinstance(sub_batch[0][k], list):
...
class SlatConditionedSLat(SlatConditionedMixin, CustomSLat):
    """
    Structured latent conditioned structured latent dataset
    """
    
    @staticmethod
    def collate_fn(batch, split_size=None):
        return CustomSLat.collate_fn(batch, split_size)


class SlatTextConditionedSLat(SlatTextConditionedMixin, CustomSLat):
    """
    Structured latent and text conditioned structured latent dataset
    """
