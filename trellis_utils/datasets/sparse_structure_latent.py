...
from .components import CustomDatasetBase, SlatConditionedMixin, SlatTextConditionedMixin
from .components import InterpSparseStructureDataset
from .. import models
...
class CustomSparseStructureLatent(SparseStructureLatentVisMixin, CustomDatasetBase):
    def __init__(self,
        roots: str,
        *,
        latent_model: str,
        ...
        if self.normalization is not None:
            self.ss_mean = torch.tensor(self.normalization['mean']).reshape(1, -1)
            self.ss_std = torch.tensor(self.normalization['std']).reshape(1, -1)

    def get_instance(self, root, scene_id, frame_id, frame_cond_id):
        ss_latent = np.load(os.path.join(root, scene_id, frame_id, 'ss_latents.npz'))
        z = torch.tensor(ss_latent['mean']).float()
        if self.normalization is not None:
            z = (z - self.ss_mean) / self.ss_std
        pack = {
            'x_0': z,
        }
        return pack
...
class SlatConditionedSparseStructureLatent(SlatConditionedMixin, CustomSparseStructureLatent):
    """
    Structured latent-conditioned sparse structure dataset
    """
    pass


class SlatTextConditionedSparseStructureLatent(SlatTextConditionedMixin, CustomSparseStructureLatent):
    """
    Structured latent and text-conditioned sparse structure dataset
    """
    pass


class InterpSparseStructureLatent(SparseStructureLatentVisMixin, InterpSparseStructureDataset):
    ...
    def __init__(self,
        ...
        *,
        ...
        self.normalization = normalization
        self.value_range = (0, 1)
        self.interp_ss_num = interp_ss_num
    ...
    def load_ss_cond(self, root, scene_id, frame_id):
        ss_latent = np.load(os.path.join(root, scene_id, frame_id, 'ss_latents.npz'))
        z = torch.tensor(ss_latent['mean']).float()
        if self.normalization is not None:
            z = (z - self.ss_mean) / self.ss_std
    ...
    def get_instance(self, root, scene_id, frame_ids):
        
        L = len(frame_ids)
        start = np.random.randint(0, L - 2 * self.interp_ss_num)
        frames = np.arange(start, start + self.interp_ss_num * 2, 2)

        x_0 = torch.stack([self.load_ss_cond(root, scene_id, frame_ids[frame]) for frame in frames], dim=0).float()
        pack = {
            'x_0': x_0, # [4, 8, 16, 16, 16]
            'cond': 0,  # No text condition for interpolation
        }
        return pack

    @torch.no_grad()
    def decode_latent(self, z, batch_size=4):
        # randomly choose one frame to decode
        n_frames = z.shape[1]
        idx = torch.randint(0, n_frames, (z.shape[0],), device=z.device)
        z = z[torch.arange(z.shape[0]), idx]
        return super().decode_latent(z, batch_size=batch_size)
