...
from ..modules.sparse.transformer import ModulatedSparseTransformerCrossBlock, TwoCondModulatedSparseTransformerCrossBlock
from .sparse_structure_flow import TimestepEmbedder
from .sparse_elastic_mixin import SparseTransformerElasticMixin


class SimpleSparseResBlock3d(nn.Module):
    def __init__(
        self,
        channels: int,
...
class TwoConditionedSLatFlowModel(nn.Module):
    def __init__(
        self,
        resolution: int,
        in_channels: int,
        ...
        self.blocks = nn.ModuleList([
            TwoCondModulatedSparseTransformerCrossBlock(
                model_channels,
    ...
    def forward(self, x: sp.SparseTensor, t: torch.Tensor, cond: List) -> sp.SparseTensor:
    ...
    def preprocess_cond(self, cond) -> Tuple[sp.SparseTensor, torch.Tensor]:
        """
        Preprocess the condition tensor.
        """
        slat_cond, text_cond = cond
        ...
        if self.pe_mode == "ape":
            slat_cond = slat_cond + self.pos_embedder(slat_cond.coords[:, 1:]).type(self.dtype)
...
class ElasticTwoConditionedSLatFlowModel(SparseTransformerElasticMixin, TwoConditionedSLatFlowModel):
    pass
