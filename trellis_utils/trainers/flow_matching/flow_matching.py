...
from .mixins.slat_conditioned import SlatConditionedMixin, SlatTextConditionedMixin
# from .mixins.interp_conditioned import InterpConditionedMixin
...
class FlowMatchingDiffusionForcingTrainer(FlowMatchingTrainer):

    def training_losses(self, x_0, cond=None, **kwargs) -> Tuple[Dict, Dict]:
        
        bs = x_0.shape[0]
        x_0 = rearrange(x_0, 'b t c x y z -> (b t) c x y z')
        noise = torch.randn_like(x_0)
        t = self.sample_t(x_0.shape[0]).to(x_0.device).float()
        x_t = self.diffuse(x_0, t, noise=noise)
        cond = self.get_cond(cond, **kwargs)

        x_0 = rearrange(x_0, '(b t) c x y z -> b t c x y z', b=bs)
        x_t = rearrange(x_t, '(b t) c x y z -> b t c x y z', b=bs)
        noise = rearrange(noise, '(b t) c x y z -> b t c x y z', b=bs)
        t = rearrange(t, '(b t) -> b t', b=bs)
        pred = self.training_models['denoiser'](x_t, t * 1000, cond, **kwargs)
        assert pred.shape == noise.shape == x_0.shape
        target = self.get_v(x_0, noise, t)
        terms = edict()
        ...
        # log loss with time bins
        pred = rearrange(pred, 'b t c x y z -> (b t) c x y z')
        target = rearrange(target, 'b t c x y z -> (b t) c x y z')
        mse_per_instance = np.array([
            F.mse_loss(pred[i], target[i]).item()
            for i in range(pred.shape[0])
        ])
        time_bin = np.digitize(t.cpu().numpy().reshape(-1), np.linspace(0, 1, 11)) - 1
        for i in range(10):
            if (time_bin == i).sum() != 0:
                terms[f"bin_{i}"] = {"mse": mse_per_instance[time_bin == i].mean()}
...
class SlatConditionedFlowMatchingCFGTrainer(SlatConditionedMixin, FlowMatchingCFGTrainer):
    pass


class SlatTextConditionedFlowMatchingCFGTrainer(SlatTextConditionedMixin, FlowMatchingCFGTrainer):
    
    @staticmethod
    def pack_sparsetensor(x: List[sp.SparseTensor]) -> sp.SparseTensor:
        coords = []
        feats = []
        for i, x_i in enumerate(x):
            assert x_i.shape[0] == 1, "Only support 1 SparseTensor each to be packed"
            coord = x_i.coords[:, 1:]
            feat = x_i.feats
            coords.append(torch.cat([torch.full((coord.shape[0], 1), i, dtype=torch.int32, device=coord.device), coord], dim=-1))
            feats.append(feat)
        coords = torch.cat(coords)
        feats = torch.cat(feats)
        packed_x = sp.SparseTensor(
            coords=coords,
            feats=feats,
        )
        return packed_x
    ...
    def get_cond(self, cond, **kwargs):
        ...
        batch_size = len(cond) // 2
        text_cond = torch.stack(cond[batch_size:], dim=0)
        slat_cond = self.pack_sparsetensor(cond[:batch_size])
        return [slat_cond, text_cond]
    
    def get_inference_cond(self, cond, **kwargs):
        args = super().get_inference_cond(cond, **kwargs)
        packed_args = {}
        for k, v in args.items():
            if isinstance(v, list):
                batch_size = len(v) // 2
                text_cond = torch.stack(v[batch_size:], dim=0)
                slat_cond = self.pack_sparsetensor(v[:batch_size])
                packed_args[k] = [slat_cond, text_cond]
            else:
                packed_args[k] = v
        return packed_args
