...
class SlatConditionedSparseFlowMatchingCFGTrainer(SlatConditionedMixin, SparseFlowMatchingCFGTrainer):
    ...
    def get_cond(self, cond, **kwargs):
        ...
        return self.pack_sparsetensor(cond)
    ...
    def get_inference_cond(self, cond, **kwargs):
        ...
        for k, v in args.items():
            if isinstance(v, list):
                packed_args[k] = self.pack_sparsetensor(v)
            else:
                packed_args[k] = v
        return packed_args


class SlatTextConditionedSparseFlowMatchingCFGTrainer(SlatTextConditionedMixin, SparseFlowMatchingCFGTrainer):

    def get_cond(self, cond, **kwargs):
        cond = super().get_cond(cond, **kwargs)
        batch_size = len(cond) // 2
        text_cond = torch.stack(cond[batch_size:], dim=0)
        slat_cond = SlatConditionedSparseFlowMatchingCFGTrainer.pack_sparsetensor(cond[:batch_size])
    ...
    def get_inference_cond(self, cond, **kwargs):
        ...
        for k, v in args.items():
            if isinstance(v, list):
                ...
                slat_cond = SlatConditionedSparseFlowMatchingCFGTrainer.pack_sparsetensor(v[:batch_size])
