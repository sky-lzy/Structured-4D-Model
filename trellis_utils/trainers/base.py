...
class Trainer:
    ...
    def __init__(self,
        ...
        **kwargs
        ...
        if num_workers is None:
            ...
            num_workers = int(np.ceil(int(os.environ.get("SLURM_CPUS_ON_NODE", os.cpu_count())) / num_gpus))
    ...
    def run(self):
        ...
        while self.step < self.max_steps:
            ...
            # Sample images
            if self.enable_snapshots and self.i_sample is not None and self.i_sample > 0 and self.step % self.i_sample == 0:
                self.snapshot()
            ...
            if self.is_master:
                ...
                if isinstance(log[-1][1]["grad_clip"]["max_norm"], np.float32):
                    log[-1][1]["grad_clip"]["max_norm"] = log[-1][1]["grad_clip"]["max_norm"].astype(np.float64)
